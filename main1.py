...
with open(chemin_fichier, 'r') as fichier:
    # Lisez chaque ligne et ajoutez-la à la liste
    for ligne in fichier:
        diseases.append(ligne.strip())  # Utilisez strip() pour supprimer les caractères de nouvelle ligne ('\n')
...
@app.get("/")
async def root():
 return {"greeting":"Disease predidtion"}
...
async def predict_disease_from_symptom(symptom_list: List[str]):
        ...
        # Obtention des 5 maladies les plus probables
        top_5_indices = np.argsort(probabilities[0])[::-1][:5]
        result = []

        for i in top_5_indices:
            result.append(diseases[i])
